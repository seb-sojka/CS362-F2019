...
int main() 
{
	...
	int handPos = 0;
	printf("Test 1: Minion choose neither options\n");
	...
	int retValue = cardEffect(minion, 0, 0, 0, &testGame, 0, 0);
	printf("return effect value = %d, expected = %d\n", retValue, -1);
	if(retValue != -1)
	{
		printf("Minion card call return correct with 0\n");
	}
	else
	{
		printf("Minion card call return error with -1\n");
	}
	printf("Expected returned to be -1");
